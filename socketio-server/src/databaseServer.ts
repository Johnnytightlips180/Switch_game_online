  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      if (match) {
        ...
        // Generate JWT storing users id and username
...
// Asynchronous function to update user statistics after a game
export const updateUserStatsRoute = async (req: Request, res: Response) => {
  // Get the user ID and statistics from the request body
  const { userID, stats } = req.body;

  // Check that the user ID and statistics are not empty
  if (!userID || !stats) {
    res.status(400).json({ message: 'Missing required fields' });
    return;
  }
  ...
  try {
    // Calculate win loss ratio, consider when losses are 0
    let winLossRatio = stats.losses !== 0 ? stats.wins / stats.losses : stats.wins;

    // Update the user statistics in the database
    await pool.query(
      'UPDATE user_records SET wins = ?, losses = ?, win_loss_ratio = ? WHERE id = ?',
      [stats.wins, stats.losses, winLossRatio, userID]
    );

    // Respond with a success message
    res.json({ message: 'User statistics updated successfully' });
  } catch (error) {
    // Log and respond with an error if the query fails
    console.error(error);
    res.status(500).json({ message: 'Internal server error' });
  }

  // Log the incoming update request for debugging purposes
